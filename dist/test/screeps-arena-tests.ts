...
export function loop(): void {
    ...
    // $ExpectType Creep[]
    const enemyCreeps = getObjectsByPrototype(prototypes.Creep).filter((i) => !i.my);
    ...
    if (myTower) {
        ...
        const findClosestByRange = myTower.findClosestByRange(getObjectsByPrototype(Creep).filter((i) => !i.my));
        ...
        // TODO: findPathTo with options
        // $ExpectType Creep
        const utilsFindClosestByPathResult = findClosestByPath(myTower, enemyCreeps);
        // TODO: findClosestByPath with options

        if (enemyFlag) {
            const positions: Array<Creep | Flag> = [...enemyCreeps, enemyFlag];
            // $ExpectType (Creep | Flag)[]
            const findInRangeMultipleTypesOfPositions = myTower.findInRange(positions, 1);
        }
    }
